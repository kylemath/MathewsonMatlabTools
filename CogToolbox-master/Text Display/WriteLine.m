    ...
    while ~isempty(paragraphs{i}) % write this whole paragraph
      ...
      if any(~ismember(nextWord,delimiters)) % at least 1 non-blank character here
       ...
       if textend(1) <= rect(3)- margin
            [x, y]=DrawFormattedText(win,[nextWord ' '],x, y, writecolor);  %Added by ATran2019
          else
            [x, y]=DrawFormattedText(win,[nextWord ' '],margin, y+floor(size*linespacing), writecolor);  %Added by ATran2019
          end
      end
