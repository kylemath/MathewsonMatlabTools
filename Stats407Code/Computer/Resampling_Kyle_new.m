...
% It will show a basic t-test, then a between subjects bootstrap and
% jacknife of the same data
% than the same for a correlation model
...
%Basic old school t-test of the data
difference = left_amygdala-right_amygdala;
[significance p_value] = ttest(difference, zeros(n_subjects,1));  %Just a regular paired t-test
    t_value = tinv(p_value,n_subjects-1);  
...
figure;
    ...
    subplot(1,3,3);
        hist(difference,n_subjects);
            title(['Size Difference (t(' num2str(n_subjects-1) ') = ' num2str(t_value) ', p = ' num2str(p_value) ')']);
...
[stat ids] = bootstrp(n_samples,'mean',both);  %This is the secret
    boot_mean = mean(stat)
